      ...
      <div className="flex items-center gap-4">
        ...
        <div className="relative" ref={dropdownRef}>
          ...
          >
            ...
            {count > 0 && (
              <span
                className={`absolute -top-1.5 -right-1.5 flex h-5 min-w-5 items-center justify-center rounded-full px-1 text-[10px] font-bold text-white ${
          ...
          {isOpen && (
            <div className="absolute right-0 top-full mt-2 w-80 sm:w-96 rounded-2xl border border-slate-200 bg-white shadow-lg overflow-hidden">
              <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3">
                <h3 className="text-sm font-semibold text-slate-900">Notifications</h3>
                {count > 0 && (
                  <span className="text-xs text-slate-500">{count} alert{count !== 1 ? "s" : ""}</span>
                )}
              </div>
